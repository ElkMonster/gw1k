...
-- Afterwards, we assume that directory creation succeeded (or it already existed before), 
-- so we attempt to create some files in there (i.e., no error checks here)
local tmp_dir_name = ".themetmp"
os.execute("mkdir " .. tmp_dir_name)

-- This is the file that contains the theme definition; theme_name will be set from ThemeManager.
-- The theme file content will be copied into a file that creates a Lua module of the theme definition.
-- This module is then loaded by the actual theme program in order to create all color key-value pairs.
local theme_filename = "themes/" .. theme_name .. ".theme"

-- This is the name of the program that will be executed in order to create all color key-value pairs
